...
> 文档更新：2026-09-18
> 源码核对基线：GPU-8B2d B2d-4a 工作区（2026-09-18）
> 阶段状态：GPU-0 ～ GPU-8A 已完成相应 checkpoint；GPU-8B2c Phase 4/5 已实现 retained VIEWMODEL span marker、独立 depth/coverage、Post fog skip、CPU/GPU differential 与 local muzzle 接入；GPU-8B2d B2d-4a 已把普通 RFM2 material alpha 接入真实 flat/textured producer，local muzzle/其他透明 world producer 与 GPU-9A normal-frame 验收仍待完成。
...
- normal world batch 必须整批可表达才使用 GPU；B2d-4a 的普通 RFM2 material alpha 可由 flat/textured source-over command 表达，transparent、edge、overlay 或 other 仍导致整批 CPU fallback。
