...
int main(void)
{
    ...
    assert(snapshot.actors[1].identity.generation == 1);
    input[3].active = 0;
    input[6] = input[8];
    input[8].active = 0;
    assert(rf_gpu_scene_snapshot_build_v1(&tracker, input, &camera,
    ...
    input[6].active = 0;
    assert(rf_gpu_scene_snapshot_build_v1(&tracker, input, &camera,
        14, 1, 1280, 720, &snapshot) == 0 && snapshot.actor_count == 0);
    input[8].active = 1;
    assert(rf_gpu_scene_snapshot_build_v1(&tracker, input, &camera,
        15, 1, 1280, 720, &snapshot) == 0);
    assert(snapshot.actors[0].identity.generation == 3);
    assert(rf_gpu_scene_snapshot_build_v1(&tracker, input, &camera,
        16, 1, 1280, 720, &snapshot) == 0 && snapshot.actor_count == 1);
    input[3].active = 1;
    input[3].source = RF_GPU_SCENE_ACTOR_NETWORK;
    input[3].source_id = 2;
    assert(rf_gpu_scene_snapshot_build_v1(&tracker, input, &camera,
        17, 1, 1280, 720, &snapshot) == 0);
    assert(snapshot.actors[0].identity.generation == 4);
    assert(rf_gpu_scene_snapshot_build_v1(&tracker, input, &camera,
        18, 2, 1280, 720, &snapshot) == 0);
    assert(snapshot.actors[0].identity.generation == 5);
    assert(snapshot.actors[1].identity.generation == 6);
    assert(rf_gpu_scene_snapshot_build_v1(&tracker, input, &camera,
        18, 2, 1280, 720, &snapshot) == -1);
    replay = snapshot;
