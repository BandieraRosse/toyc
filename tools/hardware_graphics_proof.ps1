...
if (([int]$MixedGate.IsPresent + [int]$DepthGate.IsPresent + [int]$CacheGate.IsPresent + [int]$ExecutorGate.IsPresent) -gt 1) { throw 'Choose one gate.' }
...
if ($ExecutorGate -and -not $PSBoundParameters.ContainsKey('Executable')) {
    $Executable = 'build-windows/rasterfall-gpu-mixed-test.exe'
}
$Root = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
if (-not $OutputDirectory) {
    $Prefix = if ($ExecutorGate) { 'tmp/hg2b-executor-' } elseif ($CacheGate) { 'tmp/hg2b-cache-' } elseif ($MixedGate) { 'tmp/hg2b-mixed-bridge-' } elseif ($DepthGate) { 'tmp/hg2b-depth-bridge-' } else { 'tmp/hg2a-' }
...
$Checkpoint = if ($ExecutorGate) { 'HG-2B-core-executor' } elseif ($CacheGate) { 'HG-2B-registry-cache' } elseif ($MixedGate) { 'HG-2B-mixed-bridge' } elseif ($DepthGate) { 'HG-2B-depth-bridge' } else { 'HG-2A' }
[string[]] $ProofArguments = if ($CacheGate -or $ExecutorGate) { @() } elseif ($MixedGate) { @('--mixed-gate') } elseif ($DepthGate) { @('--depth-gate') } else { @(Join-Path $OutputDirectory 'proof') }
...
$Manifest = [ordered]@{
    ...
    boundary = 'hosted offscreen graphics and Raster ABI interop; diagnostic final readback; Core mixed executor has a separate ExecutorGate; native consumer not implemented'
}
try {
    ...
    $ArtifactNames = if ($DepthGate -or $MixedGate -or $CacheGate -or $ExecutorGate) { @('proof.log','proof.stderr.log') } else { @('proof.ppm','proof.depth-f32','proof.log') }
    ...
    $PassMarker = if ($ExecutorGate) { 'core-mixed-executor: PASS' } elseif ($CacheGate) { 'registry-gpu-cache: PASS' } elseif ($MixedGate) { 'HG-2B mixed bridge: PASS' } elseif ($DepthGate) { 'HG-2B depth prerequisite: PASS' } else { 'HG-2A graphics proof: PASS' }
