...
static int png_to_ttex(const char*in,const char*out,uint32_t target_w,uint32_t target_h){
 size_t n,pos=8,raw_n=0,cap=0;unsigned char*p=load(in,&n),*raw=NULL,*idat=NULL,*pix=NULL;uint32_t w=0,h=0;int bd=0,ct=0,inter=0,channels=3;int rc=-1;
 ...
 {int bpp=(ct==2?3:ct==6?4:ct==0?1:2),stride=w*bpp;uLongf z=(uLongf)(h*(stride+1));channels=(ct==6||ct==4)?4:3;raw=malloc((size_t)z);if(!raw||uncompress(raw,&z,idat,raw_n)!=Z_OK||z!=h*(stride+1))goto done;pix=malloc((size_t)w*h*channels);if(!pix)goto done;
  for(uint32_t y=0;y<h;y++){unsigned char*cur=raw+y*(stride+1)+1,*prev=y?raw+(y-1)*(stride+1)+1:NULL;int f=raw[y*(stride+1)];for(int x=0;x<stride;x++){int a=x>=bpp?cur[x-bpp]:0,b=prev?prev[x]:0,c=prev&&x>=bpp?prev[x-bpp]:0;switch(f){case 1:cur[x]+=a;break;case 2:cur[x]+=b;break;case 3:cur[x]+=(unsigned char)((a+b)/2);break;case 4:{int q=a+b-c,pa=abs(q-a),pb=abs(q-b),pc=abs(q-c);cur[x]+=(unsigned char)(pa<=pb&&pa<=pc?a:pb<=pc?b:c);break;}case 0:break;default:goto done;}}for(uint32_t x=0;x<w;x++){unsigned char*r=cur+x*bpp,*d=pix+((size_t)y*w+x)*channels;d[0]=r[0];d[1]=ct==0?r[0]:ct==4?r[0]:r[1];d[2]=ct==0?r[0]:ct==4?r[0]:r[2];if(channels==4)d[3]=r[bpp-1];}}
 }
 {uint32_t ow=target_w?target_w:w,oh=target_h?target_h:h;unsigned char*outpix=NULL;FILE*f;
  ...
  if(ow!=w||oh!=h){outpix=malloc((size_t)ow*oh*channels);if(!outpix)goto done;
   for(uint32_t y=0;y<oh;y++)for(uint32_t x=0;x<ow;x++){uint32_t sx=x*w/ow,sy=y*h/oh;memcpy(outpix+((size_t)y*ow+x)*channels,pix+((size_t)sy*w+sx)*channels,channels);}
  }else outpix=pix;
  f=fopen(out,"wb");if(!f){if(outpix!=pix)free(outpix);goto done;}fwrite("TTEX",1,4,f);p16(f,1);p16(f,32);p32(f,ow);p32(f,oh);p16(f,channels);p16(f,1);p32(f,32);p32(f,ow*oh*channels);p32(f,0);fwrite(outpix,1,(size_t)ow*oh*channels,f);fclose(f);if(outpix!=pix)free(outpix);rc=0;}
done: free(p);free(idat);free(raw);free(pix);return rc;
}

static int bmp_to_ttex(const char*in,const char*out){
 size_t n;unsigned char*p=load(in,&n),*pix=NULL;uint32_t w,h,off,row;int top_down,bpp,rc=-1;
 if(!p||n<54||memcmp(p,"BM",2)||g32(p+14)<40)goto done;
 w=g32(p+18);h=g32(p+22);off=g32(p+10);bpp=g16(p+28);top_down=(int32_t)h<0; if(top_down)h=(uint32_t)-(int32_t)h;
 if(!w||!h||w>8192||h>8192||off>=n||(bpp!=24&&bpp!=32)||g32(p+30)!=0)goto done;
 row=(w*bpp/8+3)&~3U;if((uint64_t)off+(uint64_t)row*h>n)goto done;pix=malloc((size_t)w*h*3);if(!pix)goto done;
 for(uint32_t y=0;y<h;y++){uint32_t sy=top_down?y:h-1-y;const unsigned char*s=p+off+(size_t)sy*row;for(uint32_t x=0;x<w;x++){const unsigned char*q=s+x*bpp/8;unsigned char*d=pix+((size_t)y*w+x)*3;d[0]=q[2];d[1]=q[1];d[2]=q[0];}}
 {FILE*f=fopen(out,"wb");if(!f)goto done;fwrite("TTEX",1,4,f);p16(f,1);p16(f,32);p32(f,w);p32(f,h);p16(f,3);p16(f,1);p32(f,32);p32(f,w*h*3);p32(f,0);fwrite(pix,1,(size_t)w*h*3,f);fclose(f);rc=0;}
done:free(p);free(pix);return rc;
}
...
static int validate(const char*path){size_t n;unsigned char*p=load(path,&n);int ok=0;if(!p||n<32)goto done;if(!memcmp(p,"TTEX",4)&&g16(p+4)==1&&(g16(p+16)==3||g16(p+16)==4)&&g16(p+18)==1&&g32(p+20)==32&&g32(p+24)==n-32)ok=1;else if(!memcmp(p,"TSND",4)&&g16(p+4)==1&&g16(p+14)==16&&g32(p+20)==32&&g32(p+24)==n-32&&g32(p+16)<=0xffffffffu/(g16(p+12)*2u))ok=1;else if(!memcmp(p,"TMES",4)&&g16(p+4)==1&&g32(p+16)==16&&g32(p+20)==4&&g32(p+24)==40&&g32(p+28)>=40+g32(p+8)*16&&g32(p+12)%3==0)ok=1;done:free(p);return ok?0:-1;}
static int inspect(const char*path){size_t n;unsigned char*p=load(path,&n);if(!p)return -1;printf("%s: %zu bytes, ",path,n);if(!memcmp(p,"TTEX",4)&&n>=32)printf("TTEX v%u %ux%u %s\n",g16(p+4),g32(p+8),g32(p+12),g16(p+16)==4?"RGBA8888":"RGB888");else if(!memcmp(p,"TSND",4)&&n>=32)printf("TSND v%u %u Hz %u ch %u frames\n",g16(p+4),g32(p+8),g16(p+12),g32(p+16));else if(!memcmp(p,"TMES",4)&&n>=40)printf("TMES v%u %u vertices %u indices\n",g16(p+4),g32(p+8),g32(p+12));else{free(p);return -1;}free(p);return 0;}
static void usage(void){fprintf(stderr,"usage: toyasset convert <png|png1024|bmp|jpg|jpg128|wav|obj> input output | inspect file | validate file\n");}
int main(int argc,char**argv){if(argc==3&&!strcmp(argv[1],"inspect"))return inspect(argv[2]);if(argc==3&&!strcmp(argv[1],"validate"))return validate(argv[2]);if(argc==5&&!strcmp(argv[1],"convert")){if(!strcmp(argv[2],"png"))return png_to_ttex(argv[3],argv[4],0,0)?1:0;if(!strcmp(argv[2],"png1024"))return png_to_ttex(argv[3],argv[4],1024,1024)?1:0;if(!strcmp(argv[2],"bmp"))return bmp_to_ttex(argv[3],argv[4])?1:0;if(!strcmp(argv[2],"jpg"))return jpg_to_ttex(argv[3],argv[4],0,0)?1:0;if(!strcmp(argv[2],"jpg128"))return jpg_to_ttex(argv[3],argv[4],128,128)?1:0;if(!strcmp(argv[2],"wav"))return wav_to_tsnd(argv[3],argv[4])?1:0;if(!strcmp(argv[2],"obj"))return obj_to_tmesh(argv[3],argv[4])?1:0;}usage();return 2;}
