...
static int png_to_ttex(const char*in,const char*out){
...
static int jpg_to_ttex(const char*in,const char*out){size_t n;unsigned char*p=load(in,&n),*pix=NULL;uint32_t w=0,h=0;int rc=-1;if(!p)goto done;if(jpg_to_rgb(p,n,&pix,&w,&h)<0||!pix)goto done;{FILE*f=fopen(out,"wb");if(!f)goto done;fwrite("TTEX",1,4,f);p16(f,1);p16(f,32);p32(f,w);p32(f,h);p16(f,3);p16(f,1);p32(f,32);p32(f,w*h*3);p32(f,0);fwrite(pix,1,(size_t)w*h*3,f);fclose(f);rc=0;}done:free(p);free(pix);return rc;}
...
static void usage(void){fprintf(stderr,"usage: toyasset convert <png|jpg|wav|obj> input output | inspect file | validate file\n");}
int main(int argc,char**argv){if(argc==3&&!strcmp(argv[1],"inspect"))return inspect(argv[2]);if(argc==3&&!strcmp(argv[1],"validate"))return validate(argv[2]);if(argc==5&&!strcmp(argv[1],"convert")){if(!strcmp(argv[2],"png"))return png_to_ttex(argv[3],argv[4])?1:0;if(!strcmp(argv[2],"jpg"))return jpg_to_ttex(argv[3],argv[4])?1:0;if(!strcmp(argv[2],"wav"))return wav_to_tsnd(argv[3],argv[4])?1:0;if(!strcmp(argv[2],"obj"))return obj_to_tmesh(argv[3],argv[4])?1:0;}usage();return 2;}
