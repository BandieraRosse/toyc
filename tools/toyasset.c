...
static void usage(void){fprintf(stderr,"usage: toyasset convert <png|png128|jpg|wav|obj> input output | inspect file | validate file\n");}
int main(int argc,char**argv){if(argc==3&&!strcmp(argv[1],"inspect"))return inspect(argv[2]);if(argc==3&&!strcmp(argv[1],"validate"))return validate(argv[2]);if(argc==5&&!strcmp(argv[1],"convert")){if(!strcmp(argv[2],"png"))return png_to_ttex(argv[3],argv[4],0,0)?1:0;if(!strcmp(argv[2],"png128"))return png_to_ttex(argv[3],argv[4],128,128)?1:0;if(!strcmp(argv[2],"jpg"))return jpg_to_ttex(argv[3],argv[4])?1:0;if(!strcmp(argv[2],"wav"))return wav_to_tsnd(argv[3],argv[4])?1:0;if(!strcmp(argv[2],"obj"))return obj_to_tmesh(argv[3],argv[4])?1:0;}usage();return 2;}
