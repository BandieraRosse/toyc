#!/usr/bin/env python3
"""Capture and assemble the real-map character distance sheet.

The resulting layout matches real.png: near/mid/far rows and old/idle/aim/
motion columns.  All cells come from the normal world renderer.
"""
...
def read_bmp(path):
    ...
    if dib < 40 or planes != 1 or compression != 0 or bpp not in (24, 32):
...
def assemble(paths, cell_width):
    ...
    sheet_width, sheet_height = cell_width * 4, cell_height * 3
    ...
    return sheet_width, sheet_height, sheet


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--model", default="rasterfall/private-assets/models/rf_humanoid_v2.rmesh")
    parser.add_argument("--output", default="tmp/rf-humanoid-v2/character-world.png")
    ...
    expected = [capture_dir / f"{distance}-{pose}.bmp"
                for distance in DISTANCES for pose in POSES]
    if not args.capture_dir:
        subprocess.run([args.rasterfall, "--character-world-capture", str(capture_dir),
                        "--character-world-model", args.model], check=True)
    ...
    print(f"character world sheet: {output}")
