...
内容地图（空间地图与 World Content 的合并工程视图）使用同一环境：
...
`world-layout` 输出内容地图的 `output.json` 和 `layout.png`；`map-layout` 只输出空间地图。
...
`<id>_lodN.rmesh`。导入流程和空间规范见[资产导入指南](../docs/rasterfall/guides/asset-pipeline.md)，
字段合同见[manifest 参考](../docs/rasterfall/reference/asset-manifest.md)及
