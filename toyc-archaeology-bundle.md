...
以上为作者当日回答的忠实摘要。缺少独立材料支持的部分维持口述或推断等级。

## 未纳入的对象与证据边界

本轮没有恢复已删除 `offline` ref 的确切 tip，没有纳入 XN6、AVX、SD Rust 的固定源码快照、
完整正式赛果与排行榜，也没有取得 BusyBox/libctest/LTP 的原始比赛镜像和输出。因此相关来源、
赛事评价、项目影响和实际测试结果均保持正文注明的有限证据等级，不作为后续主动任务。
...
[Tinylibc 证据表](../evidence/tinylibc.md)，跨时期结论见[关键问题台账](../questions.md)。
...
musl，系统调用内联汇编与宏机制则是在理解后加入并留下详细注释。本轮没有固定当时参考的 musl
commit，因此不对逐文件复制和改写比例作进一步结论。
...
以下材料未取得，因此相关细节维持当前证据等级，不作为本轮后续任务：

1. 2025 年参考的 musl 精确快照，以及 syscall、内联汇编、pthread/clone 的逐文件许可对照。
2. 课程提交包或报告、毕业论文原文和线程实验原始输出。
3. 2026 年 3 月前未提交的音频实验。
4. 6 月末 Claude Code 配置与 DeepSeek API 原始会话。

Tinylibc `87d61e0` 与 ToyCCompiler 根提交的提取快照和文件级差异已经完成核对，见
[ToyCCompiler 证据表](../evidence/toy-c-compiler.md)，不再列为缺口。
...
本章以 Rasterfall 成为仓库治理主线为终点。`toyar`、浮点、库兼容与其他应用扩展已经作为
连接整合期与 Rasterfall 的背景纳入叙事，不再另立必须完成的考古章节。
