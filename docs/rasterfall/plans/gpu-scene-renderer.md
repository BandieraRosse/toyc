...
敌人导航作为阶段 4 逻辑帧时间优化独立切片；2026-09-26 改用共享目标导航场，按下文专节验收。
...
确定性 720 tick 回归确认查询量只减少约一成，大量地面采样仍在主路径。最终候选降低 64 档逻辑成本与
realtime 长尾，但满载仍未达到 60 FPS，且仍存在丢时。
...
| 导航搜索/路径可通行检查 | 按下文共享目标导航场复用反向搜索与物理连接，并对扫掠候选做保守范围筛选 | 允许改变寻路选择和队形；保持真实通行、权威所有权和确定性，缓存按碰撞版本与通行能力失效 |
...
当前行为调整：优先让整波敌人持续行动。复用已有分层格/节点作区域和出口，个体直接采用共享方向，
实际到达后发布同格同层指路记录；无路线时低成本探索，局部失败不立即重启全场。
保留有界共享搜索处理复杂地形，允许绕路与较慢的目标刷新。首版不另建门户图。
用户要求快速交付，仅构建和必要逻辑回归；游戏效果由用户实测，不重复三轮 GPU 性能验收。
下述性能现场属于此前等待接入的版本，不能作为本次行为调整的性能结论。

2026-09-26 用户批准替换集团导航。以下实现切片已接入并通过 Windows 逻辑回归：

1. 复用玩法移动规则，建立支持同格多支撑面及障碍/坡道关键节点的共享图。
2. 按目标共享可续算反向搜索；敌人从自身位置接入，不再集结或追赶领队路线。
3. 搜索、接入和修复采用确定性全局配额；缓存满排队，禁止回退逐敌同步完整搜索。
4. 覆盖绕墙、窄门、上下层、坡道、目标迁移、地图失效、容量、公平性与确定性；运行 Windows
   原生 build/test，并以正式西侧走廊 32/64 敌人比较查询量与到达行为。

默认普通导航通行半径显式统一为普通敌人半径，延续旧方案允许混合体型共用通道的行为意图，
取消对建组领队的依赖；冲锋等技能的实际碰撞尺度仍由技能规则负责。
验收必须同时报告覆盖与等待、搜索和物理采样、到达率及逻辑耗时，不以暂时等待代替性能改善。
集团机制仅作诊断对照。实现事实归[玩法架构](../architecture/gameplay.md#敌人共享目标导航场)。

同一目标 32/64 人共用一份场，正常路径不调用旧搜索；扫掠范围候选及平地证明保留严格运动验证后备，
