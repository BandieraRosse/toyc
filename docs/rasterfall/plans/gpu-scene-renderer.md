...
> 方向调整：2026-09-24
> 当前切片：阶段 4，西侧走廊 32/64 敌人优化；先归因逻辑尖峰和资源更新，再实施裁剪、热点优化与顶点压缩
...
| 导航搜索/路径可通行检查 | 先复用查询工作区并避免重复初始化；再按测量考虑缓存 | 缓存键包含导航/碰撞 generation、端点、半径和高度；不近似复用不同位置，不改变寻路选择顺序 |
| 特感索敌、技能或队友更新 | 缓存同一 tick 内完全相同的只读查询，按实际热点缩小扫描 | 查询结果依赖的状态变化时失效；保留目标选择、技能时序及确定性 |

每项保留原实现作为诊断对照。相同 seed、命令、固定 tick 下逐 tick 比较权威状态和事件，
覆盖拥挤、门切换、坡道、击飞、死亡、切图；先通过逻辑回归，再测 realtime。
未测出热点的候选不实施，不同时重写导航、碰撞和 AI。
...
1. 正确性与算法成本采用同 package 的固定输入对照。逻辑改动比较相同 tick 的权威状态；
