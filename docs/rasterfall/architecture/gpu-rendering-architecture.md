...
冻结来源及其展示历史仍按原顺序推进。Scene 在敌人和程序角色几何提取前对保守的身体与附属内容联合范围做视锥排除；
只有范围确定落在近、左、右、上或下平面外时跳过提取、光照和上传。阴影与 Smoker 舌头、目标束缚圈进入范围；
导入感染体按 recipe、bind/步态首次完成提取后缓存该姿态实际蒙皮顶点的宽松范围；范围尚未建立时继续提取。
这个逐帧判断不改变透明顺序，也不将资源容量槽当作跨帧角色身份。
