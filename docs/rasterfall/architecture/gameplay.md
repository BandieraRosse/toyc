...
导航路径采样复用同一高度碰撞检查返回的地面结果，复用范围仅限同一位置、半径及当前高度的
单次调用；不缓存跨采样点或跨 tick 的结果，不改变地图失效规则与候选路径顺序。
高度碰撞先排除不与当前 footprint 相交的高台，再判断其与坡道的接缝；仍按原图元顺序判定。
敌人近距离追击现先调用 `toy_game_short_connection()`：查询限制在 2400 RFU 内，按实际半径、
起始地面高度、连续支撑、墙体和安全室检查直达。成立时省去该次逐敌人网格搜索；失败时仍走
现有导航路径。诊断记录 `nav_short_queries` 和 `nav_short_reachable`，可与搜索及路径采样量一起
判断查询成本。集团集结、共享路线和可续算搜索尚未接入。
