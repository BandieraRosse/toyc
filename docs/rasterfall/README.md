...
| 独立 Scene 运行成本、动态资源及角色上传复用 | [活动计划](plans/README.md)、[GPU 架构](architecture/gpu-rendering-architecture.md)、[Scene 工作流](guides/gpu-scene-fixture.md) | `tools/gpu_scene_cost.ps1`、`tools/gpu_scene_cost_report.py`、`rf_gpu_graphics_triangle_resource_update`、`rf_gpu_graphics_skinned_resource_update`；同包交替比较资源和上传策略 |
