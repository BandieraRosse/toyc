...
“以后或许写个简化的 C 编译器”的同期文字。6 月 29 日会话 `ls-712f9fec88e391ac0047bd5c`
又直接说明这个想法约一个月前形成，主要目的就是让 Tinylibc 真正自举，甚至考虑以后摆脱
Makefile；经历内核项目后，作者判断编译器比完整 Linux 内核更现实。这把 compiler 固定为
Tinylibc 自包含链条的一环，而非 agent 新提出的独立方向。直接触发因素仍是 Claude Code 与
DeepSeek API 带来的效率提升，使此前不敢投入的目标看起来可行。

同一会话也是 Tinylibc 项目哲学形成的同期现场。作者正在追问：若只是追赶 glibc 的功能与兼容，
项目意义何在？讨论逐渐收敛为透明地展示底层机制、让代码和应用可较快读懂、由实际需求驱动功能、
逐步取得构建与运行独立性，以及不靠压行数把复杂度藏到别处。这些原则随后进入 README、
CLAUDE.md 等文本；自举编译器正是“独立”仍缺失的一环。
...
测试策略并非一次突然出现。6 月 30 日 agent 宣称可变参数宏和浮点等能力完成时，作者在
`ls-f602149a47d21b50e15a5c66` 立即追问为何没有测试文件；实际运行 `test_va` 又暴露死循环和
持续增长的内存。这先确立了“实现存在不等于 feature 完成，能力声明必须有独立测试证据”。

7 月 2 日 `ls-b92a5a5a9dcee2d0ef1d0c95` 中，作者明确说三十多条提交后的编译器复杂度已经到了
自己“不能控制”的程度，并要求先写文档，再用一个个小测试确定能力边界。遇到深层错误时，推进
方式也从继续追大型 self-host binary 的段错误，转为先构造针对 tcc 自身写法的局部、可解释、
可复现 failure。`2e03788`
加入编译器测试套件，`4437887` 合并 `compiler-test-suite` 工作线，`fddf85c`、`ff396cb` 整理阶段式
运行器，7 月 3 日随后出现针对短路求值、指针算术、参数、typedef、多维数组和库编译的密集修复。
Git 提交显示策略如何落地，会话则直接保存了其决策来源。因此这不宜简称为“改用 TDD”：测试从
功能证明发展为能力清单、回归边界、失败定位和对 agent 修改的独立验收，也是作者重新取得工程
控制的认知工具。
...
在 6 月末突然启动，也解释了为什么作者需要把测试逐步提升为能力边界、故障隔离和 self-host scope
的主要控制手段。
...
4. 6 月末 Claude Code 与 DeepSeek API 的底层配置和模型调用日志；关键开发会话已经由 Agent
   History Corpus 恢复，但不能由此还原每次模型请求或所有未保存上下文。
