...
7 月 4 日同期会话 `ls-ae12d0ff80d1bb2004f69072` 直接记录作者主动建立新 tcc 项目，并说明
“可以编译部分 C 文件，但编译自身还有问题”。随后同一会话建立独立 Makefile、带入 `elf.h`、
抽出最小 `tcc_need.h`，避免继续依赖 Tinylibc 的 `syscall.h` 与 `tlibc_everything.h`，并马上继续
self-host 测试。因此独立建仓可直接确认为作者主动建立的工程边界和目标收缩，而非公开展示包装。

作者 2026-09-15 回忆，集中上下文也是拆分考虑之一，并称当时已计划日后重新整合 Tinylibc；这两项
目前没有在 7 月 4 日同期会话中找到直接表述，必须保留为后期回忆。同期材料只能确认：编译器仍有
self-host 问题、建立了独立构建与最小依赖边界，并随即集中推进 self-host。`Toyc` 名称后来才确定。
...
这延续并收紧了 Tinylibc 末期的测试策略。agent 一度把 suite 扩为约 94 个 generic compiler tests；
`ls-ae12d0ff80d1bb2004f69072` 中作者质疑数量与可理解性，随后直接叫停，规定所有测试都应帮助 tcc
正确编译自身，只覆盖 tcc 源码真实使用的语言模式；tcc 源码可以为易自举而简化，测试则不应为了
全部通过而掩盖真实 compiler bug。agent 随后把方案收缩为围绕自身源码模式的小组测试。

因此测试哲学不是一次“转向 TDD”，而是从证明 feature，发展为测量能力边界、隔离 self-host
failure，最终收缩到 self-host 真正需要的语言模式。“先建测试框架”及 scope 收缩已有同期
agent-session 直接证据，不再只是作者回忆；Git 提交仍用于确认这些决策怎样落地。
