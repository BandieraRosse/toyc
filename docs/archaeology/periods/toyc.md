...
6 月 29 日同期会话已经确认，编译器从一开始就是为 Tinylibc self-host 而生。7 月 4 日独立建仓
时，同期可确认的是编译器仍不能正确编译自身、项目建立最小依赖边界并集中推进 self-host；“为了
集中 Claude 上下文”及“当日已计划合回”仍是后期回忆，不能写成同期既定计划。

目前可证的最早强 agent-session 统一证据在 7 月 11 日：`ls-d1bf96ba7a8f02510e526f28` 中作者
明确要求新简介列出两个旧仓库，说明 Toyc 将与 Tinylibc 合并、成为独立生态，tcc 是重要组成部分。
因此到这一天，项目身份上的重新统一已经明确；这不等于断言该想法绝对首次诞生于当天。结合原始
self-host 动机，统一是重新连接原来的目标，而不是把两个无关成果事后拼接。
...
现有自然开发 prompt 没有发现 literal “two parents”，但这不削弱同日 Git 证据：`643287b` 的
英文 README 明写 “either parent”，提交标题也写 “two parents”。prompt 证明作者提出合并与
生态规划，Git 对象证明该措辞当日已经进入项目文本；两类证据并列而不互相替代。
