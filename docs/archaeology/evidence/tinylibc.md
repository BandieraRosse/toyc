...
| 编译器起点 | `4670d9f` | 2026-06-30 | 最小编译器 Phase 1 | 目标与初始规划已有完整会话；逐项实现归因仍不能只凭 transcript 或 trailer |
| 测试策略转折 | `2e03788`、`4437887`、`ff396cb` | 2026-07-02 | 测试支线合并、阶段式 runner | 决策来源已有同期 agent-session 直接证据 |
...
Agent History 将这条策略从作者回忆升级为同期证据。`ls-712f9fec88e391ac0047bd5c`（6 月 29 日）
直接记录 compiler 为 Tinylibc self-host 服务及透明、可读、独立的项目取向；
`ls-f602149a47d21b50e15a5c66`（6 月 30 日）记录作者质疑没有测试文件的 feature 完成声明，并在
实际运行中发现 `test_va` 死循环；`ls-b92a5a5a9dcee2d0ef1d0c95`（7 月 2 日）记录“三十多条提交、
复杂度不能控制”，以及先写文档、再用小测试确定能力边界的明确要求。Git 提交仍用于确认这些决策
怎样落地；session 证明的是目标、问题定义和验证要求，不把每项测试或实现整体归给作者。
...
本轮未纳入课程材料、论文原文和输出、当时的 musl 快照、未提交音频实验及文章旧 Git 对象。
关键 compiler 开发会话已由 Agent History Corpus 恢复，但并不覆盖所有模型调用或未保存上下文。
Tinylibc 编译器树到 ToyCCompiler 根提交的逐文件对照已经完成，见
