...
2026-09-15，作者允许考古计划参考本机旧文档目录 `draft/` 中的文章草稿和发布稿。
...
## 2026-09 Codex 访谈原始会话

2026-09-16 从本机 Codex rollout 中恢复了 2026-09-15 考古访谈的作者原始输入。正式归档只选取
无法由 Git 重建、且直接解释项目动机、贡献来源与工作方式的回答，见
[作者访谈摘录](oral-history/2026-09-15-author-interview.md)。会话 UUID 与提取、编辑规则记录在
该文件的来源说明中；完整 rollout 不复制进仓库，因为它同时包含系统提示、工具输出、重复总结和
与考古无关的环境信息。

这些回答属于事后回忆。摘录中的 `Cross-check` 是后来的 Git/材料核查，不是原回答的一部分；
作者的犹豫、记忆空白和反事实判断均按原证据等级保留。Codex 或其他 assistant 对回答的即时概括
不作为独立来源。
...
- 完整 Claude Code / ToyCCompiler 对话正文、客户端配置、内嵌提示词、命令历史和显式署名策略。
  本轮已检查可恢复的 Claude `history.jsonl` prompt index 与少量项目 `.claude/` 同期资料；这些
  零散材料不能替代已经缺失的完整关键开发会话。
