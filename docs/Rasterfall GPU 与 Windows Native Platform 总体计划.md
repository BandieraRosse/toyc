...
> 当前状态：A-AUDIT、B1-CONTRACT、B2-SKY、B3-WORLD 与 B4-POST-WORLD submission contract 已实现；effects/viewmodel 已有独立 pre-post barrier，screen overlay 有唯一 target。transparent 与 post-world GPU consumer 仍未实现。等待 Windows Intel normal-frame、路径切换与 resize 实机验收。GPU-8B1/GPU-9A 尚未冻结。
...
- 定义并实现 GPU-8B2，将 viewmodel、interactables 和 world effects 等 renderer command layers 纳入 GPU frame，
