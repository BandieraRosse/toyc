...
- `toyc` — C 编译器（350 KB，git 追踪）
- `toyas` — x86_64 汇编器（74 KB，git 追踪）
- `toyld` — x86_64 静态链接器（33 KB，git 追踪）

## 用途

`bootstrap/toyc` + `bootstrap/toyas` + `bootstrap/toyld` 是 Makefile 的默认工具链，
...
这会用当前源码构建 toyc+toyas+toyld，然后覆盖 `bootstrap/` 中的种子。
通常在源码重大变更后执行。
