 ...
 *
 * 本测试来自调试 tas.c 自举编译的过程。tas.c 的 add_sym() 使用
 *   sym_names[elf_sym_count][ni] = name[ni];
 * 模式（static char sym_names[256][128]），tcc 曾在此处生成错误代码。
 *
 * 根因：tcc 对全局 2D 数组的第一维步长计算为 sizeof(char)=1 而非
 *   sizeof(char[128])=128，导致 sym_names[i] 访问到错误行。
 * 该 bug 在 parse.c 中有部分修复（gv_elem_at 计算），但旧版自举种子
 * bootstrap/tcc-boot3 不包含该修复，因此 tas.c 最终将 sym_names 改为
 * 1D 数组 + 手动偏移来彻底规避。
 *
 * 本测试验证 tcc 对 char[256][16] 的读写是否正确。
