/*
 * 改编自 https://github.com/lieff/minimp3 (public domain)
 *
 * 针对 ToyCCompiler 环境的适配：
 *   - 移除所有标准库 #include（类型由 tlibc_types.h 提供）
 *   - 移除所有 SIMD 代码路径（HAVE_SIMD=0, HAVE_SSE=0）
 *   - 移除 Layer I/II 支持（仅保留 Layer III/MP3）
 *   - memcpy/memset/memmove 由 toyc_rt + lib 提供
 *
 * 注意事项：
 *   - 使用前必须包含 tlibc_types.h（提供 uint8_t, int16_t, size_t 等）
 *   - memcpy/memset/memmove 在链接时解析（toyc_rt.c / lib/string.c）
 */
...
static void L3_read_scalefactors(uint8_t *scf, uint8_t *ist_pos,
    const uint8_t *scf_size, const uint8_t *scf_count, bs_t *bitbuf, int scfsi)
{
    int i, k;
...
static void L3_decode_scalefactors(const uint8_t *hdr, uint8_t *ist_pos,
    bs_t *bs, const L3_gr_info_t *gr, float *scf, int ch)
{
...
static void L3_stereo_process(float *left, const uint8_t *ist_pos,
    const uint8_t *sfb, const uint8_t *hdr, int max_band[3], int mpeg2_sh)
{
...
static void L3_imdct_gr(float *grbuf, float *overlap,
    unsigned block_type, unsigned n_long_bands)
{
...
static int L3_restore_reservoir(mp3dec_t *h, bs_t *bs,
    mp3dec_scratch_t *s, int main_data_begin)
{
...
static void L3_decode(mp3dec_t *h, mp3dec_scratch_t *s,
    L3_gr_info_t *gr_info, int nch)
{
    int ch;
    for (ch = 0; ch < nch; ch++)
    {
...
static void mp3d_synth_granule(float *qmf_state, float *grbuf,
    int nbands, int nch, mp3d_sample_t *pcm, float *lins)
{
    int i;
    for (i = 0; i < nch; i++)
    ...
    mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    {
        frame_size = hdr_frame_bytes(mp3, dec->free_format_bytes)
                   + hdr_padding(mp3);
        if (frame_size != mp3_bytes
            && (frame_size + HDR_SIZE > mp3_bytes
...

    {
        ...
        if (success)
        {
                 ...
                 igr++, pcm += 576*info->channels)
            {
                ...
                L3_decode(dec, &scratch, scratch.gr_info
                    + igr*info->channels, info->channels);
                mp3d_synth_granule(dec->qmf_state, scratch.grbuf[0],
                    18, info->channels, pcm, scratch.syn[0]);
            }
        }
