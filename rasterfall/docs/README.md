...
> 文档更新：2026-09-18
> 源码核对基线：GPU-0 ～ GPU-8A 已完成；GPU-8B2d 达到 local pass。Windows Intel normal-frame audit 已确认 identity/fog 支持场景 native present 与零 readback/copy，并补齐 vertex-lit source-over；正常 Campaign 角色 toon/material feature 仍以 `0x40` 整帧 CPU replay，随后释放 native GPU ownership 并锁存 compatibility renderer，以保证 fallback ownership 安全，因此 GPU-8B1、GPU-9A 尚未冻结。
