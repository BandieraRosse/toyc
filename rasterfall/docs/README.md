...
> 文档更新：2026-09-16
> 源码核对基线补充：GPU-2 已完成 Core-owned `rf_gpu` contract 与持久 Vulkan loader/instance/device/queue backend；hosted probe 经正式 service 验证 compute/readback 和 shutdown，正常 runtime 仍 disabled/CPU renderer，GPU framebuffer 属于 GPU-3。
...
| GPU 枚举、Core GPU service、持久 Vulkan backend、compute ownership 与 fallback | [build-platforms.md](build-platforms.md)、[`../../gpu/README.md`](../../gpu/README.md) | `include/rf_gpu.h`、`src/rf_gpu.c`、`gpu/src/rf_gpu_vulkan_backend.c`、`gpu/src/rf_gpu_probe.c`；`make gpu-service-test` / `make gpu-probe` / `make win-gpu-probe`，当前不接正常 renderer |
