...
> 源码核对基线补充：GPU-8B2c Phase 1+2 已冻结 Viewmodel Render Contract V1（near=192、focal=3/4、真近平面裁剪、独立 inverse-Z depth、Post coverage mask），并在 CPU/reference 路径建立 domain switch 与固定 fixture；weapon/hands/pill producer、GPU VIEWMODEL consumer、local muzzle 与 transparent 仍待后续 checkpoint。
