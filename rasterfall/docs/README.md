...
> 文档更新：2026-09-06
> 源码核对基线：工作区（本地玩家移动/跳跃/airborne/水平朝向/武器/库存/动画/special-control/shove 由 `actors[0]` 直接驱动；空格/E 等关键边沿经过固定步 pending 队列；生成距离、安全室/战役目标、敌人近战/Tank 受击、爆炸目标、敌人特感索敌、投掷物/药丸、hitscan 开火和 update_held 武器推进已按 actor 规则处理；legacy 仅供兼容入口和展示适配；shove 兼容入口按字段冲突选择导入范围，不全量覆盖 actor；camera body 由 actor 派生；网络协议未变）
