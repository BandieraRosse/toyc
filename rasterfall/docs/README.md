...
> 文档更新：2026-09-17
> 源码核对基线：GPU-0 ～ GPU-8A 已完成；RenderFrame V1 已建立单调层 cursor，normal frame 按 sky/world/transparent/effects/viewmodel/overlay 契约提交；Core 在 viewmodel barrier 统一决定 GPU 提交或整帧 CPU replay。GPU-8B2 审计已区分每层 command、direct pixels 和 fallback reason，纯 Raster V1 effects command 可随 retained stream 消费；effects facade 已独立回传真实 direct producer 统计；billboard 与普通 particle 已迁入 opaque raster commands，本地 tracer 等屏幕线 ray direct producer、transparent 与 viewmodel 仍待迁移。GPU-8B1、GPU-9A 仍待 Windows normal-frame 冻结。
