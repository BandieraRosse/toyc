...
> 文档更新：2026-09-16
> 源码核对基线补充：GPU-2A 已建立 Core-owned `rf_gpu` contract、optional/required fallback、状态查询与 backend 生命周期；正常 runtime 仍 disabled/CPU renderer，持久 Vulkan backend 属于待完成 GPU-2B。
...
| GPU 枚举、Core GPU service、Vulkan compute ownership 与 fallback | [build-platforms.md](build-platforms.md)、[`../../gpu/README.md`](../../gpu/README.md) | `include/rf_gpu.h`、`src/rf_gpu.c`、`gpu/src/rf_gpu_probe.c`；`make gpu-service-test` / `make gpu-probe` / `make win-gpu-probe`，当前不接正常 renderer |
