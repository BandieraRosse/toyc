...
> 文档更新：2026-09-18
> 源码核对基线：GPU-0 ～ GPU-8A 已完成；RenderFrame V1 已建立单调层 cursor，normal frame 按 sky/world/transparent/effects/viewmodel/overlay 契约提交；Core 在 viewmodel barrier 统一决定 GPU 提交或整帧 CPU replay。GPU-8B2d B2d-0..4e 已完成 Transparent V1 语义、retained span、显式 fallback reason 与 RFM2/platform/air-gate/muzzle/dissolve/death fragment/dust 真实 producer；B2d-5 完整 normal-frame fixture 已固定 WORLD/EFFECTS/VIEWMODEL 的 opaque+transparent 组合、两个 ABI barrier、零 direct debt/fallback reason 和 native hand-off，GPU-8B2 达到 local pass。GPU-8B1、GPU-9A 仍待 Windows normal-frame 冻结。
> 当前 GPU 优先级：在 Windows Intel 实机验收 GPU-8B1/GPU-9A 的 normal native frame，包括 sky/VIEWMODEL 路径切换、Console/Desktop、resize、timing 与零 readback；`--frame-audit` 已同时写入 `rasterfall.log`。详见 [GPU 与 Windows Native Platform 阶段计划](../../docs/Rasterfall%20GPU%20%E4%B8%8E%20Windows%20Native%20Platform%20%E6%80%BB%E4%BD%93%E8%AE%A1%E5%88%92.md)。
