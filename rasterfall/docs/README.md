...
> 文档更新：2026-09-18
> 源码核对基线：GPU-0 ～ GPU-8A 已完成；RenderFrame V1 已建立单调层 cursor，normal frame 按 sky/world/transparent/effects/viewmodel/overlay 契约提交；Core 在 viewmodel barrier 统一决定 GPU 提交或整帧 CPU replay。GPU-8B2 审计已区分每层 command、direct pixels 和 fallback reason，纯 Raster V1 effects command 可随 retained stream 消费；billboard、普通 particle 与屏幕线 ray 已迁入携带投影逆深度的 opaque raster commands，`effects_direct_pixels=0` 门禁已建立；GPU-8B2c Phase 4 已让 GPU consumer 按 retained VIEWMODEL span marker 切换独立 depth/coverage 域，transparent 与 local muzzle 仍待迁移。GPU-8B1、GPU-9A 仍待 Windows normal-frame 冻结。
> 当前 GPU 优先级：在 Windows 实机验收 sky 与 VIEWMODEL 的 CPU/GPU 路径切换一致性，并继续迁移 transparent、local muzzle；`--frame-audit` 已同时写入 `rasterfall.log`。详见 [GPU 与 Windows Native Platform 阶段计划](../../docs/Rasterfall%20GPU%20%E4%B8%8E%20Windows%20Native%20Platform%20%E6%80%BB%E4%BD%93%E8%AE%A1%E5%88%92.md)。
> 源码核对基线补充：GPU-8B2c Phase 1+2 已冻结 Viewmodel Render Contract V1（near=192、focal=3/4、真近平面裁剪、独立 inverse-Z depth、Post coverage mask），并在 CPU/reference 路径建立 domain switch 与固定 fixture；Phase 3 已将 weapon/hands/pill normal producer 收敛为共享 flat/lit/textured triangle commands，direct framebuffer producer 门禁为 0；Phase 4 已接入 GPU VIEWMODEL span consumer、独立 depth/coverage 与 Post fog skip，并由 CPU/GPU differential fixture 覆盖。local muzzle 与 transparent 仍待后续 checkpoint。
