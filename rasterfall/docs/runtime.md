...
> 当前验收边界：Windows Intel strict native smoke、Fog/Post smoke 与零 readback/copy 已通过；但 GPU-8B1/GPU-9A 尚未冻结。Legacy anime normal rendering 已冻结并回退 humanoid，原 toon/material `0x40` 不再是 normal frame 输入；Console/Desktop normal runtime 也已隔离，F12、反引号和 station 交互只产生 HUD 暂时不可用提示。retained command 堆越界已修复，继续验收 pause/resume、resize、timing、native present 与正常退出。Windows `--frame-audit` 同步写 `rasterfall.log`，`--normal-frame-audit` 用于精确重放。
