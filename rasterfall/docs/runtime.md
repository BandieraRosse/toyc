...
> 文档更新：2026-09-17
> 源码核对基线：默认 CPU；显式 `--renderer gpu-compute` 启用 Core-owned normal GPU frame，`--gpu-native-present` 启用零 readback swapchain 路径；RenderFrame V1 以单调 cursor 强制六层顺序，Core 按层保留 pre-post command 并在 viewmodel barrier 统一决策 GPU 提交或整帧 CPU replay，之后才进入唯一 screen overlay target。
