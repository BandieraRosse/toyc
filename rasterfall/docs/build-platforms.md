...
> 文档更新：2026-09-17
> 源码核对基线：Windows normal binary 已链接共享 Vulkan backend、Raster V1、Texture V1、Post-Raster V1、overlay composite 和 Win32 swapchain presentation；Core 在 viewmodel barrier 之前按层保留 pre-post command，未迁移的 effects/viewmodel 输入会使整帧按原批次 CPU replay，防止 native present 丢失层；默认仍为 CPU，GPU 由命令行显式选择。
