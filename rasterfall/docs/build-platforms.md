...
> 文档更新：2026-09-20
> 源码核对基线补充：2026-09-20 HG-2C5 以 Windows Native Vulkan 为唯一 GPU 维护平台；Phase 0 保留 hot queue-idle 并加入 `PRESENT-AUDIT`。后续 validation gate 必须区分 `validation unavailable` 与零错误 PASS；WSL/llvmpipe 不进入正确性、兼容性或性能验收。
