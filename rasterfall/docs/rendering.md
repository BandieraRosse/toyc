...
RGBA 分类；B2d-4a/4b 已按真实 RFM2 material alpha 与 RGBA texel alpha
迁移普通 flat/textured world producer，并保留 RGB + material alpha=255 的 opaque 快路径；RGBA command 在 CPU replay 与 GPU pack 前即具有 no-depth-write，真实 fixture 覆盖 alpha 0/128/255、RGB 对照、texture table、retained span 和 hand-off；muzzle outer/lobe 与 enemy dissolve death fade 已迁移，后续再冻结 material × texel 组合 alpha producer。VIEWMODEL transparent 使用同一
