...
#define NET_WORLD_BURN_ZONE_SIZE 16
#define NET_WORLD_SIZE (NET_WORLD_FIXED_SIZE + 2 + \
...
{
    return rasterfall_net_transport_now_ms();
}
                                            ...
                                            struct rasterfall_session *session)
{
    ...
    for (i = 0; i < net->remote_event_count; i++) {
        ...
        if (event->type == RASTERFALL_NET_EVENT_PLAYER_IMPULSE) {
            ...
            if (actor->special_control !=
                    TOY_GAME_SPECIAL_CONTROL_NONE &&
                actor->special_control_id >= event->control_id)
                continue;
            toy_game_set_actor_special_control(
                toy_game_local_player_actor(game),
                TOY_GAME_SPECIAL_CONTROL_SMOKER, event->control_id,
            ...
            toy_game_clear_actor_special_control(
                toy_game_local_player_actor(game), event->control_id);
        }
    }
    net->remote_event_count = 0;
}
                                   ...
                                   struct toy_game *game)
{
    ...
    for (i = net->local_event_scan_count; i < game->event_count; i++) {
        ...
        net->reliable_events[net->reliable_event_count].x =
            toy_game_local_player_actor_const(game)->x;
        net->reliable_events[net->reliable_event_count].z =
            toy_game_local_player_actor_const(game)->z;
        net->reliable_events[net->reliable_event_count].value =
            toy_game_local_player_actor_const(game)->current_slot;
...
static int net_send(struct rasterfall_net *net, unsigned char *packet, int size)
{
    ...
    sent = rasterfall_net_transport_send(net->fd, &net->server_address,
                                         packet, size);
    if (sent == size) {
                       ...
                       unsigned char *packet, int size)
{
    ...
    if (!net || !address || net->fd < 0 || size <= 0 ||
        size > RASTERFALL_NET_MAX_PACKET) return -1;
    if (net_simulated_drop(net, packet_type_of(packet, size))) return 0;
    sent = rasterfall_net_transport_send(net->fd, address, packet, size);
    if (sent == size) {
        net->net_stats_tx_bytes += (unsigned long)size;
        net->net_stats_tx_packets++;
        net_stats_roll(net);
...
static int net_client_actor_index(const struct rasterfall_net_client *client);
static struct toy_game_actor *net_client_actor(
    struct toy_game *game, const struct rasterfall_net_client *client);
...
static int net_client_actor_index(const struct rasterfall_net_client *client)
{
    if (!client || client->client_id <= 0 ||
        client->client_id > RASTERFALL_NET_CLIENT_MAX) return -1;
    return TOY_GAME_REMOTE_ACTOR_BASE + client->client_id - 1;
}

static struct toy_game_actor *net_client_actor(
    struct toy_game *game, const struct rasterfall_net_client *client)
{
    int index = net_client_actor_index(client);
    if (!game || index < 0 || index >= TOY_GAME_MAX_ACTORS) return NULL;
...
static int punch_packet(const unsigned char *p, int size, int type)
{
    return size >= 6 && p[0] == 'R' && p[1] == 'F' && p[2] == 'P' &&
           p[3] == '3' && p[4] == RASTERFALL_PUBLIC_VERSION && p[5] == type;
}
...
static int punch_send_register(struct rasterfall_net *net)
{
    ...
    packet[0] = 'R'; packet[1] = 'F'; packet[2] = 'P'; packet[3] = '3';
    ...
    packet[10] = (unsigned char)(net->mode == RASTERFALL_NET_HOST ?
        RASTERFALL_PUBLIC_ROLE_HOST : RASTERFALL_PUBLIC_ROLE_GUEST);
    packet[11] = 0;
    return rasterfall_net_transport_send(net->fd, &net->public_server,
                                         packet, sizeof(packet)) ==
           (long)sizeof(packet) ? 0 : -1;
}
...
static int punch_send_probe(struct rasterfall_net *net)
{
    ...
    packet[0] = 'R'; packet[1] = 'F'; packet[2] = 'P'; packet[3] = '3';
    packet[4] = RASTERFALL_PUBLIC_VERSION;
    packet[5] = RASTERFALL_PUBLIC_PROBE;
...
{
    net->fd = rasterfall_net_transport_open(0, 0);
    return net->fd >= 0 ? 0 : -1;
}
                        ...
                        const struct camera *spawn)
{
    ...
    net->fd = rasterfall_net_transport_open(port, 1);
...
int rasterfall_net_connect(struct rasterfall_net *net, const char *ip, int port)
{
    ...
    if (net->server_address.sin_addr.s_addr == 0xffffffffU) {
        rasterfall_net_transport_close(net->fd);
        net->fd = -1;
        return -1;
    }
    net->server_known = 1;
...
int rasterfall_net_public_host(struct rasterfall_net *net, int room_id,
                               const struct camera *spawn)
{
    if (!rasterfall_public_room_valid(room_id)) return -1;
    rasterfall_net_init(net);
    ...
    net->relay_mode = rasterfall_public_room_uses_relay(room_id);
...
void rasterfall_net_close(struct rasterfall_net *net)
{
    if (net && net->mode != RASTERFALL_NET_OFF)
        __printf("rasterfall net: rtt=%dms input tx/rx=%lu/%lu "
                 ...
                 "entity/world=%lu/%lu reconcile=%lu "
                                ...
                                int jump_dx, int jump_dz)
{
        ...
        net->pending_shop_input_sequence = sequence;
    {
        ...
        const struct toy_game_actor *actor = local_game ?
            toy_game_local_player_actor_const(local_game) : NULL;
    ...
    put_i16(p + 24, predicted->y);
    {
        const struct toy_game_actor *actor = local_game ?
            toy_game_local_player_actor_const(local_game) : NULL;
        put_i16(p + 26, actor ? actor->airborne_ms : 0);
        put_i16(p + 28, actor ? actor->airborne_y : 0);
        put_i16(p + 30, actor ? actor->vertical_velocity : 0);
        put_i16(p + 32, actor ? actor->air_x : 0);
        put_i16(p + 34, actor ? actor->air_z : 0);
        put_i16(p + 36, actor ? actor->ground_y : 0);
    }
...
static void net_push_remote_sample(struct rasterfall_net *net,
                                   const struct rasterfall_net_actor *actor)
{
    int id;
    int count;
    ...
    camera.sy = actor->sy; camera.cy = actor->cy;
    camera.y = RASTERFALL_STANDING_CAMERA_Y;
    ...
    samples[count].camera = camera;
    samples[count].airborne_y = actor->airborne_y;
    net->remote_sample_count[id] = count + 1;
    if (count == 0) {
        net->remote_render_camera[id] = camera;
        net->remote_render_airborne_y[id] = actor->airborne_y;
    }
}
...
static void encode_actor(unsigned char *p, const struct toy_game_actor *a,
                        int actor_index, uint32_t input_ack)
{
    int slot = a->current_slot;
    ...
    p[43 + TOY_GAME_MAX_NAME] = (unsigned char)(input_ack >> 24);
    p[44 + TOY_GAME_MAX_NAME] = (unsigned char)(input_ack >> 16);
    p[45 + TOY_GAME_MAX_NAME] = (unsigned char)(input_ack >> 8);
    p[46 + TOY_GAME_MAX_NAME] = (unsigned char)input_ack;
    p[47 + TOY_GAME_MAX_NAME] = (unsigned char)slot;
    p[48 + TOY_GAME_MAX_NAME] = (unsigned char)((a->reloading ? 1 : 0) |
                                                (a->control_disabled ? 2 : 0));
    put_i16(p + 49 + TOY_GAME_MAX_NAME, a->reload_timer_ms);
    put_i16(p + 51 + TOY_GAME_MAX_NAME, a->throw_timer_ms);
    put_i16(p + 53 + TOY_GAME_MAX_NAME, a->weapon_switch_timer_ms);
    for (slot = 0; slot < TOY_GAME_WEAPON_SLOTS; slot++) {
        const struct toy_game_slot *s = &a->slots[slot];
        unsigned char *q = p + 55 + TOY_GAME_MAX_NAME + slot * 5;
...
static void decode_actor(const unsigned char *p, struct rasterfall_net_actor *a)
{
    ...
    a->input_ack = get_u32(p + 43 + TOY_GAME_MAX_NAME);
    a->current_slot = p[47 + TOY_GAME_MAX_NAME];
    if (a->current_slot >= TOY_GAME_WEAPON_SLOTS)
        a->current_slot = 0;
    a->reloading = (p[48 + TOY_GAME_MAX_NAME] & 1) != 0;
    a->control_disabled = (p[48 + TOY_GAME_MAX_NAME] & 2) != 0;
    a->reload_timer_ms = get_i16(p + 49 + TOY_GAME_MAX_NAME);
    a->throw_timer_ms = get_i16(p + 51 + TOY_GAME_MAX_NAME);
    a->weapon_switch_timer_ms = get_i16(p + 53 + TOY_GAME_MAX_NAME);
    {
        int slot;
        for (slot = 0; slot < TOY_GAME_WEAPON_SLOTS; slot++) {
            const unsigned char *q = p + 55 + TOY_GAME_MAX_NAME + slot * 5;
            a->slots[slot].weapon = get_weapon_value(q[0]);
            a->slots[slot].mag = get_i16(q + 1);
            a->slots[slot].reserve = get_i16(q + 3);
        }
    }
}
                                  ...
                                  uint32_t snapshot_sequence)
{
    ...
    for (kind = 0; kind < 2; kind++) {
        ...
        int i;
        for (i = 0; i < limit; i++) {
            int active = kind == 0 ? game->actors[i].active : game->enemies[i].active;
        ...
        do {
            ...
            p[5] = (unsigned char)chunk_index++;
            p[6] = (unsigned char)n;
            p[7] = (unsigned char)((count + max_entries - 1) / max_entries);
            if (!p[7]) p[7] = 1;
            for (i = 0; i < n; i++) {
                int index = indices[cursor + i];
                if (kind == 0) {
                    uint32_t ack = 0;
                    if (index >= TOY_GAME_REMOTE_ACTOR_BASE) {
                        int client_index = index - TOY_GAME_REMOTE_ACTOR_BASE;
                        if (client_index >= 0 && client_index < RASTERFALL_NET_CLIENT_MAX)
                            ack = net->clients[client_index].last_processed_input_sequence;
                    }
                    encode_actor(p + NET_ENTITY_CHUNK_BASE + i * entry_size,
                                 &game->actors[index], index, ack);
                } else {
                    encode_enemy(p + NET_ENTITY_CHUNK_BASE + i * entry_size,
                                 &game->enemies[index], index);
                }
            }
            if (net_send_clients(net, packet, size) < 0) return -1;
            cursor += n;
...
{
    int client_i;
    const struct toy_game_actor *host_actor =
        toy_game_local_player_actor_const(game);
    if (host_actor->fire_seq) {
        if (host_actor->fire_seq != net->host_fire_sent_seq) {
            net->host_fire_sent_seq = host_actor->fire_seq;
            net->host_fire_resend = 3;
        }
        if (net->host_fire_resend) {
            if (send_one_player_fire_packet(net, 0, host_actor->fire_seq,
                                            host_actor->ray_count,
                                            host_actor->rays) < 0)
                return -1;
    ...
    for (client_i = 0; client_i < RASTERFALL_NET_CLIENT_MAX; client_i++) {
        ...
        const struct toy_game_actor *actor =
            net_client_actor_const(game, client);
        if (!client->active || !client->connected || !actor ||
            !actor->active || !actor->fire_seq) continue;
        if (actor->fire_seq != net->player_fire_sent_seq[client_i]) {
            net->player_fire_sent_seq[client_i] = actor->fire_seq;
                                  ...
                                  struct rasterfall_net *net)
{
    ...
    for (i = 0; i < count; i++) {
        if (kind == 0 && net->actor_count < RASTERFALL_NET_MAX_ACTORS) {
            ...
            net_push_remote_sample(net, &net->actors[net->actor_count]);
            net->entity_actor_seen |= 1ULL << net->actors[net->actor_count].actor_index;
            net->actor_count++;
                                 ...
                                 struct rasterfall_net *net)
{
    ...
    uint32_t sequence;
    int i;
    if (size < 4 + NET_WORLD_FIXED_SIZE + 2 || size > 4 + NET_WORLD_SIZE)
        return -1;
            ...
            12 + RASTERFALL_MAX_FLAGS * NET_WORLD_FLAG_SIZE + i * 2);
    {
        ...
        memset(net->snapshot_projectiles, 0,
               sizeof(net->snapshot_projectiles));
        memset(net->snapshot_burn_zones, 0,
               sizeof(net->snapshot_burn_zones));
        projectile_count = w[cursor++];
        if (projectile_count > TOY_GAME_MAX_PROJECTILES ||
            cursor + projectile_count * NET_WORLD_PROJECTILE_SIZE + 1 > size - 4)
            return -1;
        for (i = 0; i < projectile_count; i++) {
            const unsigned char *pp = w + cursor;
            int id = pp[0];
            ...
            p->fuse_ms = get_i16(pp + 20); p->flash_ms = get_i16(pp + 22);
            p->age_ms = get_i16(pp + 24); p->bounces = get_i16(pp + 26);
            cursor += NET_WORLD_PROJECTILE_SIZE;
        }
        burn_count = w[cursor++];
        if (burn_count > TOY_CONFIG_MAX_BURN_ZONES ||
            cursor + burn_count * NET_WORLD_BURN_ZONE_SIZE != size - 4)
            return -1;
        for (i = 0; i < burn_count; i++) {
            const unsigned char *bp = w + cursor;
            int id = bp[0];
            struct toy_game_burn_zone *zone;
            ...
            cursor += NET_WORLD_BURN_ZONE_SIZE;
        }
    }
...
static int decode_player_fire(const unsigned char *payload, int size,
                              struct rasterfall_net *net)
{
    int player_id, ray_count, i;
    struct rasterfall_net_actor *actor = NULL;
                                      ...
                                      uint32_t ack)
{
    ...
    client->latest_input.airborne_ms = get_i16(payload + 26);
    client->latest_input.airborne_y = get_i16(payload + 28);
    client->latest_input.airborne_velocity = get_i16(payload + 30);
    client->latest_input.air_x = get_i16(payload + 32);
    client->latest_input.air_z = get_i16(payload + 34);
    client->reported_camera.y = get_i16(payload + 36) +
                                client->latest_input.airborne_y;
...
void rasterfall_net_poll(struct rasterfall_net *net)
{
    ...
    for (;;) {
        ...
        received = rasterfall_net_transport_receive(net->fd, &source, packet,
                                                     sizeof(packet));
        if (received < 0) {
        ...
        net_stats_roll(net);
        if (net->public_room && punch_packet(packet, (int)received,
                                             RASTERFALL_PUBLIC_REGISTERED)) {
            if (received >= RASTERFALL_PUBLIC_REGISTERED_SIZE &&
                get_u16(packet + 6) == (unsigned int)net->public_room_id) {
                net->public_registered = 1;
                if (net->mode == RASTERFALL_NET_HOST) net->connected = 1;
            }
            continue;
        }
        if (net->public_room && punch_packet(packet, (int)received,
                                             RASTERFALL_PUBLIC_ERROR)) {
            if (received >= RASTERFALL_PUBLIC_ERROR_SIZE &&
                get_u16(packet + 6) == (unsigned int)net->public_room_id) {
                net->public_error = packet[8];
                net->connected = 0;
            }
            continue;
        }
        if (net->public_room && punch_packet(packet, (int)received,
                                             RASTERFALL_PUBLIC_MATCH)) {
            ...
            /* The room number, not the coordinator response, selects the
             * transport.  A disagreement is a broken current-version server. */
            if ((packet[18] != 0) !=
                rasterfall_public_room_uses_relay(net->public_room_id)) {
                net->public_error = RASTERFALL_PUBLIC_ERROR_PROTOCOL;
                continue;
            }
        ...
        if (net->mode == RASTERFALL_NET_HOST) {
            ...
            } else if (type == RASTERFALL_NET_PLAYER_FIRE &&
                       decode_player_fire(packet + NET_HEADER_SIZE,
                                          payload_size, net) == 0) {
                /* Player ray traces are event data and are not part of the
                 * periodic player state snapshot. */
            ...
            if (type == RASTERFALL_NET_ENTITY_SNAPSHOT ||
                type == RASTERFALL_NET_WORLD_SNAPSHOT) {
                net->connected = 1;
                                   ...
                                   struct rasterfall_net_client *client)
{
    ...
    /* The actor is gameplay truth.  Client fields below are only the latest
     * input/protocol report used to advance this actor. */
                                ...
                                int rescuer_id)
{
    ...
    for (i = 0; i < RASTERFALL_NET_CLIENT_MAX; i++) {
        ...
        const struct toy_game_actor *actor = net_client_actor_const(
            &session->game_state, other);
        long dx, dz, d2;
        if (other->client_id == rescuer_id || !other->active ||
            !other->connected || !actor || !actor->active ||
                              ...
                              int target_id)
{
    ...
    for (i = 0; i < RASTERFALL_NET_CLIENT_MAX; i++)
        if (net->clients[i].client_id == target_id) {
            const struct toy_game_actor *actor = net_client_actor_const(
                &session->game_state, &net->clients[i]);
                                           ...
                                           struct rasterfall_session *session)
{
    ...
    for (i = 0; i < RASTERFALL_NET_CLIENT_MAX; i++) {
        ...
        int revive;
        {
            const struct toy_game_actor *rescuer_actor = net_client_actor_const(
                &session->game_state, rescuer);
            if (!rescuer->active || !rescuer->connected || !rescuer_actor ||
                !rescuer_actor->active ||
                rescuer_actor->state != TOY_GAME_ACTOR_ALIVE ||
        ...
        if (rescuer->local_revive_active) {
            if (target_id == 0) {
                ...
                struct toy_game_actor *target_actor = net_client_actor(
                    &session->game_state, &net->clients[target_id - 1]);
                if (target_actor)
                    target_actor->revive_progress_ms =
                        rescuer->local_revive_progress_ms;
            }
        }
    }
                                  ...
                                  struct camera *host_camera)
{
    ...
    for (i = 0; i < RASTERFALL_NET_CLIENT_MAX; i++) {
        ...
        if (next->valid && next->sequence == next_sequence) {
            int airborne_ms = client->latest_input.airborne_ms;
            int airborne_y = client->latest_input.airborne_y;
            int airborne_velocity = client->latest_input.airborne_velocity;
            int air_x = client->latest_input.air_x;
            int air_z = client->latest_input.air_z;
            ...
            client->latest_input = *next;
            client->latest_input.airborne_ms = airborne_ms;
            client->latest_input.airborne_y = airborne_y;
            client->latest_input.airborne_velocity = airborne_velocity;
            client->latest_input.air_x = air_x;
            client->latest_input.air_z = air_z;
...
int rasterfall_net_pipeline_test(void)
{
    ...
    /* Actor snapshots carry players and AI; world state is separate. */
    /* World entities use active-only lists. */
    {
        unsigned char world_packet[4 + NET_WORLD_SIZE];
        unsigned char *w = world_packet + 4;
        int cursor = NET_WORLD_FIXED_SIZE;
        ...
        put_i16(w + cursor + 20, 900); cursor += NET_WORLD_PROJECTILE_SIZE;
        w[cursor++] = 1;
        w[cursor + 0] = 5;
        put_u32(w + cursor + 2, 111); put_u32(w + cursor + 6, 222);
        put_i16(w + cursor + 10, 3000); cursor += NET_WORLD_BURN_ZONE_SIZE;
        if (decode_world_snapshot(world_packet, cursor + 4, &net) < 0 ||
            !net.snapshot_projectiles[3].active ||
     ...
     * latest network camera snapshot. */
    {
        ...
        if (net.remote_render_camera[2].x < 40 ||
            net.remote_render_camera[2].x > 60) return 11;
    }
                                 ...
                                 struct toy_game *game)
{
    ...
    for (i = 0; i < RASTERFALL_NET_CLIENT_MAX; i++) {
        ...
        actor = &game->actors[index];
        /* actor = gameplay truth; this function only creates/removes the
                                      ...
                                      struct toy_game *game)
{
    ...
    for (i = 0; i < RASTERFALL_NET_CLIENT_MAX; i++) {
        const struct toy_game_actor *actor = net_client_actor_const(
            game, &net->clients[i]);
        if (net->clients[i].active && net->clients[i].connected && actor &&
            actor->active && actor->state == TOY_GAME_ACTOR_ALIVE)
                                       ...
                                       int interact_pressed, int dt_ms)
{
    ...
    if (revive < 0) {
        ...
        {
            struct toy_game_actor *target_actor = net_client_actor(
                &session->game_state, target);
            if (target_actor)
                target_actor->revive_progress_ms =
                    net->host_revive_progress_ms;
        }
    }
}
...
void rasterfall_net_reconcile_client(struct rasterfall_net *net,
                                     ...
                                     struct camera *camera)
{
    const struct rasterfall_net_actor *own = NULL;
    (void)camera;
    ...
    dst->damage_dealt = src->damage_dealt;
         ...
         * all other actor gameplay fields above are authoritative. */
...
void rasterfall_net_update_presentation(struct rasterfall_net *net, int dt_ms)
{
    ...
    int id;
    if (!net) return;
    for (id = 0; id < net->actor_count; id++) {
        struct rasterfall_net_actor *actor = &net->actors[id];
        int duration;
        if (!actor->active || actor->actor_index == 0 ||
            !net_animation_is_transient(actor->animation.id)) continue;
        duration = toy_game_animation_info(actor->animation.id)->duration_ms;
        toy_game_animation_update(&actor->animation, dt_ms);
        if (actor->animation.time_ms >= duration)
            toy_game_animation_set(&actor->animation, TOY_GAME_ANIM_NONE);
    }
                                   ...
                                   uint32_t snapshot_sequence)
{
    ...
    cursor = NET_WORLD_FIXED_SIZE;
    {
        int projectile_count = 0, burn_count = 0;
        unsigned char *projectile_count_ptr = w + cursor++;
        for (i = 0; i < TOY_GAME_MAX_PROJECTILES; i++) {
            ...
            put_i16(pp + 20, p->fuse_ms); put_i16(pp + 22, p->flash_ms);
            put_i16(pp + 24, p->age_ms); put_i16(pp + 26, p->bounces);
            projectile_count++; cursor += NET_WORLD_PROJECTILE_SIZE;
        }
        *projectile_count_ptr = (unsigned char)projectile_count;
        {
            unsigned char *burn_count_ptr = w + cursor++;
            for (i = 0; i < TOY_CONFIG_MAX_BURN_ZONES; i++) {
                ...
                burn_count++; cursor += NET_WORLD_BURN_ZONE_SIZE;
            }
            *burn_count_ptr = (unsigned char)burn_count;
        }
    }
    world_size = cursor;
    size = packet_begin(packet, RASTERFALL_NET_WORLD_SNAPSHOT,
                        4 + world_size, ++net->send_sequence,
