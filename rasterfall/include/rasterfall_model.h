...
struct rasterfall_model_asset {
    ...
    int ik_analytic_candidate_probe;
    unsigned long ik_analytic_compatible_selected[2];
    unsigned long ik_analytic_compatible_selected_aa[2];
    unsigned long ik_analytic_compatible_unavailable[2];
    unsigned long ik_analytic_compatible_costly[2];
    unsigned long ik_analytic_compatible_extreme_selected[2];
    unsigned long ik_analytic_normal_dynamic[2];
    int ik_analytic_last_takeover_reason[2];
    double ik_analytic_last_dynamic_pole[2][3];
    double ik_analytic_last_anchored_pole[2][3];
    double ik_analytic_last_pole_anchor[2][3];
    double ik_analytic_last_pole_anchor_length[2];
    double ik_analytic_last_pole_anchor_ratio[2];
    int ik_analytic_last_pole_anchor_valid[2];
    int ik_analytic_last_pole_flipped_by_anchor[2];
    unsigned long ik_analytic_pole_anchor_valid_count[2];
    unsigned long ik_analytic_pole_flipped_by_anchor_count[2];
    double ik_analytic_last_compatible_pole[2][3];
    double ik_analytic_last_dynamic_error[2];
    double ik_analytic_last_compatible_error[2];
    int ik_analytic_last_compatible_valid[2];
    int ik_analytic_last_compatible_branch[2];
    int ik_candidate_trace_valid[2];
    int ik_candidate_trace_previous_solver[2];
    double ik_candidate_trace_h[2][3];
    double ik_candidate_trace_k0[2][3];
    double ik_candidate_trace_target[2][3];
    double ik_candidate_trace_pole[2][3];
    double ik_candidate_trace_previous_q[2][4];
    double ik_candidate_trace_dynamic_q[2][4];
    double ik_candidate_trace_dynamic_knee_q[2][4];
    int ik_candidate_trace_dynamic_branch[2];
    double ik_candidate_trace_dynamic_error[2];
    double ik_candidate_trace_compatible_q[2][4];
    double ik_candidate_trace_compatible_knee_q[2][4];
    int ik_candidate_trace_compatible_branch[2];
    double ik_candidate_trace_compatible_error[2];
    int ik_candidate_trace_compatible_valid[2];
    double ik_candidate_trace_selected_q[2][4];
    int ik_candidate_trace_selected_branch[2];
    int ik_candidate_trace_selection_reason[2];
    double ik_candidate_trace_selected_error[2];
    ...
    int ik_handoff_snapshot_valid;
    int ik_analytical_inherit_diagnostic;
    int ik_analytical_cache_valid[2];
    int ik_analytical_cache_thigh[2][3];
    int ik_analytical_cache_knee[2];
    int ik_analytical_cache_valid_before[2];
    int ik_analytical_cache_write[2];
    int ik_analytical_cache_read[2];
    int ik_analytical_cache_write_time[2];
    int ik_analytical_cache_read_time[2];
    int ik_ccd_diag_valid[2];
    double ik_ccd_diag_initial_error[2];
    double ik_ccd_diag_best_error[2];
    double ik_ccd_diag_final_error[2];
    unsigned int ik_ccd_diag_best_iteration[2];
    unsigned int ik_ccd_diag_iterations[2];
    double ik_ccd_diag_target[2][3];
    double ik_ccd_diag_initial_ankle[2][3];
    double ik_ccd_diag_final_ankle[2][3];
    int ik_best_iteration_enabled;
    ...
    int ik_previous_final_branch[2];
    double ik_previous_final_bend[2][3];
    int ik_previous_final_bend_valid[2];
    double ik_previous_final_bend_prev[2][3];
    int ik_previous_final_bend_prev_valid[2];
    double ik_previous_final_bend_ratio[2];
    int ik_previous_final_bend_source[2];
    double ik_last_analytical_bend[2][3];
    int ik_last_analytical_bend_valid[2];
    int ik_last_analytical_branch[2];
    int ik_last_analytical_branch_valid[2];
    double ik_analytic_last_anchor_confidence[2];
    int ik_analytic_last_anchor_rejected[2];
    int ik_analytic_last_anchor_reject_reason[2];
    double ik_analytic_last_anchor_conflict_dot[2];
    double ik_analytic_last_previous_bend_ratio[2];
    int ik_analytic_last_previous_bend_source[2];
    unsigned long ik_analytic_anchor_rejected_count[2];
    int ik_leg_transition_active[2];
    int ik_leg_transition_start_time[2];
    int ik_leg_transition_remaining[2];
    int ik_leg_transition_duration[2];
    int ik_leg_transition_reason[2];
    int ik_leg_transition_old_branch[2];
    int ik_leg_transition_new_branch[2];
    double ik_leg_transition_old_thigh_q[2][4];
    double ik_leg_transition_new_thigh_q[2][4];
    double ik_leg_transition_old_knee_q[2][4];
    double ik_leg_transition_new_knee_q[2][4];
    unsigned long ik_leg_hard_branch_switch_count[2];
    unsigned long ik_leg_transition_started_count[2];
    unsigned long ik_leg_transition_completed_count[2];
    unsigned long ik_leg_transition_interrupted_count[2];
    int ik_solver_return_thigh[2][3];
    int ik_solver_return_knee[2];
    double ik_solver_return_thigh_global_q[2][4];
    double ik_pregrant_thigh_global_q[2][4];
    int ik_pregrant_thigh_global_q_valid[2];
    double ik_final_thigh_global_q[2][4];
    ...
    int render_trace_solver_local[4][3];
    int render_trace_grant_local[4][3];
    int render_trace_final_local[4][3];
    double render_trace_solver_global[4][9];
    double render_trace_grant_global[4][9];
    double render_trace_final_global[4][9];
    int render_trace_valid;
    int render_trace_time_ms;
    int render_trace_write_bone;
    int render_trace_write_valid;
    double render_trace_ik_result_global_q[4];
    double render_trace_local_before_write_q[4];
    double render_trace_local_before_write_matrix[9];
    int render_trace_euler_convert[3];
    double render_trace_final_bone_q[4];
    int render_trace_write_path;
    int render_trace_write_valid_by_bone[4];
    int render_trace_write_path_by_bone[4];
    double render_trace_ik_result_global_q_by_bone[4][4];
    double render_trace_local_before_write_q_by_bone[4][4];
    double render_trace_local_before_write_matrix_by_bone[4][9];
    int render_trace_euler_convert_by_bone[4][3];
    double render_trace_final_bone_q_by_bone[4][4];
    int render_trace_previous_solver[2];
    int render_trace_transition_active[2];
    int render_trace_transition_remaining[2];
    int render_trace_transition_duration[2];
    int render_trace_transition_reason[2];
    double render_trace_transition_source_q[2][4];
    double render_trace_transition_target_q[2][4];
    int render_trace_reconciliation_active[2];
    int render_trace_reconciliation_used[2];
    int render_trace_reconciliation_unavailable[2];
    double render_trace_reconciliation_source_q[2][4];
    double render_trace_reconciliation_target_q[2][4];
    double render_trace_previous_final_thigh_q[2][4];
    double ik_solver_return_thigh_global_frame[2][9];
    int ik_solver_return_thigh_frame_valid[2];
    double ik_solver_return_knee_global_q[2][4];
    double ik_solver_return_ankle[2][3];
    double ik_solver_return_target[2][3];
    double ik_solver_return_error[2];
    double ik_c1_target[2][3];
    double ik_c1_error[2];
    int ik_handoff_c0_thigh[2][3];
    int ik_handoff_c0_knee[2];
    double ik_handoff_c0_thigh_global_q[2][4];
    double ik_handoff_c0_knee_global_q[2][4];
    double ik_handoff_c0_ankle[2][3];
    int ik_handoff_c1_thigh[2][3];
    int ik_handoff_c1_knee[2];
    double ik_handoff_c1_thigh_global_q[2][4];
    double ik_handoff_c1_knee_global_q[2][4];
    double ik_handoff_c1_ankle[2][3];
    int ik_warm_start_valid[2];
    int ik_warm_start_thigh[2][3];
    int ik_warm_start_knee[2][3];
    double ik_synthetic_offset[3];
    int ik_near_degenerate_ca_active[2];
    int ik_near_degenerate_ca_reconciled[2];
    int ik_near_degenerate_ca_unavailable[2];
    double ik_analytic_last_fullframe_secondary[2][3];
    double ik_analytic_last_reconciled_secondary[2][3];
