...
│       ├── override/   #   tcc 不兼容头文件遮蔽（目前为空，builtin 已填补）
...
./bootstrap-selfhost.sh           # 自举测试：bootstrap/tcc → stage-2 → 40 测试
...
3. **`compiler-tests/lib/override/`** — 目前为空（`__builtin_huge_val` 已由 tcc 直接支持），保留作为遮蔽备用
