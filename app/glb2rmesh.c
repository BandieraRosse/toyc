...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < material_count; i++) { unsigned char record[RASTERFALL_MODEL_MATERIAL_BYTES]; __memset(record, 0, sizeof(record)); put_u32(record, materials[i].color); put_u16(record+4, materials[i].metallic); put_u16(record+6, materials[i].roughness); if (write_all(out, record, sizeof(record)) < 0) { __close(out); return 1; } }
    for (i = 0; i < part_count; i++) { struct part *p=&parts[i]; int ps=p->pos_view.stride?p->pos_view.stride:12, ns=p->normal_view.stride?p->normal_view.stride:12, us=p->uv_view.stride?p->uv_view.stride:8, po=p->pos_view.offset+p->pos.offset, no=p->normal_view.offset+p->normal.offset, uo=p->uv_view.offset+p->uv.offset, j; for(j=0;j<p->pos.count;j++){ const unsigned char *q=bin+po+j*ps; unsigned char v[RASTERFALL_MODEL_VERTEX_BYTES]; __memset(v,0,sizeof(v)); *(int *)(v)=f_to_i(f32(q),232);*(int *)(v+4)=f_to_i(f32(q+4),232);*(int *)(v+8)=f_to_i(f32(q+8),232); if(p->normal.view>=0&&j<p->normal.count){q=bin+no+j*ns;*(short*)(v+12)=clamp_i(f_to_i(f32(q),32767),-32767,32767);*(short*)(v+14)=clamp_i(f_to_i(f32(q+4),32767),-32767,32767);*(short*)(v+16)=clamp_i(f_to_i(f32(q+8),32767),-32767,32767);} if(p->uv.view>=0&&j<p->uv.count){q=bin+uo+j*us;*(unsigned short*)(v+18)=clamp_i(f_to_i(f32(q),65535),0,65535);*(unsigned short*)(v+20)=clamp_i(f_to_i(f32(q+4),65535),0,65535);} if(write_all(out,v,RASTERFALL_MODEL_VERTEX_BYTES)<0){__close(out);return 1;} } }
