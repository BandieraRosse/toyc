...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < material_count; i++) { unsigned char record[RASTERFALL_MODEL_MATERIAL_BYTES]; __memset(record, 0, sizeof(record)); put_u32(record, materials[i].color); put_u16(record+4, materials[i].metallic); put_u16(record+6, materials[i].roughness); put_u32(record+8, 0xffffffffU); if (write_all(out, record, sizeof(record)) < 0) { __close(out); return 1; } }
