...
资产或工具的许可证说明。运行时使用的 `wall.ttex` 是仓库内提交的小型资源。
...
墙面纹理由本地准备的 `wall.jpg` 离线缩放为 128×128 后生成 `generated/wall.ttex`；
JPG 源图不随仓库分发。它来自 Poly Haven “Square Brick Floor”，按项目用户确认采用
CC0 1.0。来源 URL 和转换记录见
